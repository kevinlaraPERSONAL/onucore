...
const C = {
  ...
  red: "#e5484d", green: "#4cc38a", google: "#8a8b93", apple: "#8a8b93",
  // Semáforo de estado: bien (verde) · atención (amarillo) · mal (rojo).
  // El rojo de marca sigue siendo el acento de la UI; estos solo indican estado.
  ok: "#4cc38a", warn: "#e5a83a", bad: "#e5484d",
};
...
Si nada accionable: {"items":[]}.${userCtx()}`;
  ...
  const favoritePlaces = items.filter((i) => i.type === "place").slice().sort((a, b) => (a.title || "").localeCompare(b.title || ""));
  const savePlace = async () => {
    if (!placeDraft || !placeDraft.title) return;
    const sess = (await supabase.auth.getSession()).data.session;
    const uid2 = userId || (sess && sess.user.id); if (!uid2) return;
    const row = { id: placeDraft.id || uid(), user_id: uid2, type: "place", area: placeDraft.area || "personal", title: placeDraft.title.trim(), person: (placeDraft.category || "").trim(), detail: (placeDraft.detail || "").trim(), source: "app", done: false };
    await supabase.from("items").upsert(row);
    const norm = { id: row.id, type: "place", area: row.area, title: row.title, person: row.person, detail: row.detail, done: false };
    setItems((p) => { const idx = p.findIndex((i) => i.id === row.id); return idx >= 0 ? p.map((i, k) => k === idx ? { ...i, ...norm } : i) : [norm, ...p]; });
    setPlaceDraft(null);
  };
...
${JSON.stringify(snapshot)}`;
  ...
  const alerts = (() => {
    ...
    Object.entries(evByDay).forEach(([d, evs]) => { const du = daysUntil(d); if (evs.length >= 2 && du != null && du >= 0 && du <= 7) out.push({ kind: "conflict", color: C.warn, text: es ? `${evs.length} citas el ${fmtDate(d, lang)} — revisa que no se empalmen` : `${evs.length} appointments on ${fmtDate(d, lang)} — check for overlaps` }); });
    sItems.filter((i) => i.type === "obligation").forEach((o) => { const du = daysUntil(o.dateISO); if (du != null && du >= 0 && du <= 3 && !o.done) out.push({ kind: "bill", color: C.warn, text: (es ? `${o.title} vence ${du <= 0 ? "hoy" : "en " + du + " día(s)"}` : `${o.title} due ${du <= 0 ? "today" : "in " + du + " day(s)"}`) + (o.amount ? ` · ${money(o.amount, loc)}` : "") }); });
    ...
    if (scope !== "work") {
      ...
      carRecords.filter((r) => r.due_iso).forEach((r) => {
        ...
        out.push({ kind: "car", color: du <= 7 ? C.bad : C.warn, text: `🚗 ${nm} ${when}` });
      });
      if (vehicle && vehicle.insurance_expiry) { const du = daysUntil(vehicle.insurance_expiry); if (du != null && du <= 30) out.push({ kind: "car", color: du <= 7 ? C.bad : C.warn, text: es ? `🚗 Seguro del carro ${du < 0 ? "venció" : du === 0 ? "vence hoy" : `vence en ${du} día(s)`}` : `🚗 Car insurance ${du < 0 ? "expired" : du === 0 ? "expires today" : `expires in ${du} day(s)`}` }); }
    }
    // IRS quarterly estimated-tax dates coming up (within 7 days).
    { const yq = new Date().getFullYear(); const qd = [`${yq}-04-15`, `${yq}-06-15`, `${yq}-09-15`, `${yq + 1}-01-15`].find((d) => { const du = daysUntil(d); return du != null && du >= 0 && du <= 7; }); if (qd) out.push({ kind: "tax", color: C.bad, text: es ? `💵 Pago trimestral de impuestos: ${fmtDate(qd, lang)}` : `💵 Quarterly estimated tax due ${fmtDate(qd, lang)}` }); }
    ...
    if (scope !== "work") {
      importantDates.forEach((b) => {
        ...
        out.push({ kind: "birthday", color: du === 0 ? C.warn : C.dim, text: `🎂 ${b.title}${rel} — ${es ? "cumple" : "birthday"} ${when}` });
      });
    }
    // Subscriptions renewing in the next 3 days.
    subs.forEach((s) => { if (!s.dateISO) return; const du = daysUntil(nextRenewal(s.dateISO, s.dateLabel)); if (du != null && du >= 0 && du <= 3) out.push({ kind: "sub", color: C.warn, text: (es ? `${s.title} se renueva ${du === 0 ? "hoy" : "en " + du + " día(s)"}` : `${s.title} renews ${du === 0 ? "today" : "in " + du + " day(s)"}`) + (s.amount ? ` · ${money(Number(s.amount) || 0, loc)}` : "") }); });
    ...
    if (mExp > mInc && mExp > 0) out.push({ kind: "spend", color: C.bad, text: es ? `Este mes gastaste ${money(mExp - mInc, loc)} más de lo que ingresaste` : `This month you spent ${money(mExp - mInc, loc)} more than you earned` });
    return out;
  })();
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: SF, WebkitFontSmoothing: "antialiased" }}>
      ...
      <div style={{ maxWidth: contentMax, marginLeft: desktop ? `max(220px, calc(220px + (100% - ${contentMax + 220}px) / 2))` : "auto", marginRight: "auto", minHeight: "100vh", position: "relative", paddingBottom: desktop ? 40 : 86 }}>
        <div style={{ position: "sticky", top: 0, zIndex: 20, backdropFilter: "blur(14px)", background: "rgba(26,26,31,.72)", padding: "16px 20px 12px", borderBottom: `1px solid ${C.borderSoft}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>{desktop ? (<div style={{ fontSize: 20, fontWeight: 600, textTransform: "capitalize" }}>{t["nav_" + tab]}</div>) : (<><div style={{ display: "flex", alignItems: "center", gap: 8 }}><OcIcon size={22} ring="#8a9095" dot={C.gold} /><div style={{ color: C.gold, fontSize: 15, fontWeight: 700, letterSpacing: "0.18em" }}>AI</div></div><div style={{ fontSize: 9, letterSpacing: "0.26em", color: C.mute, marginTop: 2, textTransform: "uppercase" }}>{t["nav_" + tab]}</div></>)}</div>
      ...
      {toast && (<div className="rise" onClick={() => setToast(null)} style={{ position: "fixed", left: "50%", transform: "translateX(-50%)", bottom: 86, maxWidth: 400, width: "calc(100% - 40px)", zIndex: 40, background: C.surface, border: `1px solid ${toast.kind === "ok" ? C.ok : C.bad}`, borderRadius: 14, padding: "12px 16px", fontSize: 12.5, color: toast.kind === "ok" ? C.ok : C.bad, boxShadow: "0 14px 40px rgba(0,0,0,.45)" }}>{toast.text}</div>)}
            ...
            <div style={{ padding: "20px 20px 0" }}>
              ...
              <div style={cardS}>
                ...
                <PField label={t.f_goals}>
                  <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>{GOALS.map((g) => { const lbl = g[lang === "es" ? "es" : "en"]; return <Chip key={lbl} label={lbl} color={C.ok} on={profile.goals.includes(lbl)} onClick={() => toggleGoal(lbl)} />; })}</div>
                </PField>
                <div style={{ display: "flex", gap: 8 }}>
      ...
      {txnDraft && (
        <Sheet onClose={() => setTxnDraft(null)}>
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>{[["expense", t.add_expense], ["income", t.add_income]].map(([k, lbl]) => (<button key={k} onClick={() => setTxnDraft({ ...txnDraft, kind: k, cat: k === "income" ? "client" : "office", ded: k === "expense" })} style={{ flex: 1, padding: "10px 0", borderRadius: 12, cursor: "pointer", fontFamily: SF, fontSize: 14, fontWeight: txnDraft.kind === k ? 600 : 400, background: txnDraft.kind === k ? (k === "income" ? C.ok : C.surface2) : "transparent", color: txnDraft.kind === k ? (k === "income" ? C.bg : C.text) : C.mute, border: `1px solid ${txnDraft.kind === k ? (k === "income" ? C.ok : C.border) : C.border}` }}>{lbl}</button>))}</div>
      ...
      {placeDraft && (
        <Sheet onClose={() => setPlaceDraft(null)}>
          <div style={{ fontSize: 17, fontWeight: 600 }}>{lang === "es" ? "Lugar favorito" : "Favorite place"}</div>
          <FieldLabel>{lang === "es" ? "Nombre" : "Name"}</FieldLabel>
          <input value={placeDraft.title} onChange={(e) => setPlaceDraft((d) => ({ ...d, title: e.target.value }))} placeholder={lang === "es" ? "Ej. Guerrilla Tacos" : "e.g. Guerrilla Tacos"} style={obIn} />
          <FieldLabel>{lang === "es" ? "Categoría" : "Category"}</FieldLabel>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{[["restaurante", lang === "es" ? "Restaurante" : "Restaurant"], ["cafe", lang === "es" ? "Café" : "Café"], ["bar", "Bar"], ["coworking", "Coworking"], ["evento", lang === "es" ? "Evento" : "Event"], ["otro", lang === "es" ? "Otro" : "Other"]].map(([k, lbl]) => (<button key={k} type="button" onClick={() => setPlaceDraft((d) => ({ ...d, category: k }))} style={{ padding: "8px 14px", borderRadius: 999, cursor: "pointer", fontFamily: SF, fontSize: 13, border: `1px solid ${placeDraft.category === k ? C.gold : C.border}`, background: placeDraft.category === k ? "rgba(229,72,77,.12)" : "transparent", color: placeDraft.category === k ? C.gold : C.dim }}>{lbl}</button>))}</div>
          <FieldLabel>{lang === "es" ? "Nota (dirección, qué pedir…)" : "Note (address, what to order…)"}</FieldLabel>
          <textarea value={placeDraft.detail} onChange={(e) => setPlaceDraft((d) => ({ ...d, detail: e.target.value }))} rows={3} style={{ ...obIn, resize: "vertical", fontFamily: SF, lineHeight: 1.45 }} />
          <div style={{ display: "flex", gap: 8, marginTop: 22 }}>
            {placeDraft.id ? <button onClick={() => { deleteItem2(placeDraft.id); setPlaceDraft(null); }} style={{ ...btnGhost, flex: "0 0 auto", padding: "0 18px", color: C.red }}>{t.del}</button> : null}
            <button onClick={savePlace} disabled={!placeDraft.title} style={{ ...btnGold, flex: 1, opacity: !placeDraft.title ? 0.5 : 1 }}>{t.save}</button>
          </div>
        </Sheet>
      )}
      {dateDraft && (
        ...
        <Sheet onClose={() => setReportOpen(false)}>
          {(() => {
            ...
            return (<>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 4 }}><Sum label={t.rep_biz_income} value={money(bizInc, loc)} color={C.ok} /><Sum label={t.rep_biz_expenses} value={money(bizExp, loc)} color={C.bad} /></div>
              <div style={{ background: C.surface2, border: `1px solid ${C.goldSoft}`, borderRadius: 14, padding: "14px 16px", marginTop: 10 }}><div style={{ fontSize: 10.5, letterSpacing: "0.1em", textTransform: "uppercase", color: C.gold }}>{t.rep_ded}</div><div className="num" style={{ fontSize: 28, fontWeight: 300, color: C.ok, marginTop: 4 }}>{money(yDed, loc)}</div></div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 12, padding: "12px 16px", marginTop: 10 }}><span style={{ fontSize: 13, color: C.dim }}>{t.rep_net}</span><span className="num" style={{ fontSize: 18, fontWeight: 600, color: netBiz >= 0 ? C.ok : C.bad }}>{money(netBiz, loc)}</span></div>
              ...
              {(() => {
                ...
                return (<>
                  ...
                  <div style={cardS}>{forms1099.map((f) => {
                    ...
                    const col = done ? C.ok : diff > 0 ? C.warn : C.dim;
...
function Today({ t, lang, loc, briefing, briefingLoading, regenerate, events, tasks, reminders, obligations, recentId, toggleDone, onEdit, alerts, askQ, setAskQ, askA, askLoading, onAsk, clearAsk }) {
  ...
  const bills = obligations.filter((o) => { const d = daysUntil(o.dateISO); return d != null && d <= 10 && !o.done; });
  ...
  return (<>
    ...
    {empty ? <Card><Empty>{t.today_clear}</Empty></Card> : <>
      ...
      {bills.length > 0 && <Card title={t.today_bills} count={bills.length}>{bills.map((o) => { const d = daysUntil(o.dateISO); return <ItemRow key={o.id} it={o} lang={lang} recent={o.id === recentId} onOpen={onEdit} right={money(o.amount, loc)} sub={d <= 0 ? t.due_today : t.due_in.replace("{n}", d)} subColor={d <= 3 ? C.bad : C.warn} />; })}</Card>}
    </>}
  </>);
}
...
function Money({ t, lang, loc, txns, obligations, subs, period, setPeriod, fseg, setFseg, recentId, onEditTxn, onEditItem, onAdd, onAddSub, onAddObl, onTogglePaid, onReport, setAsidePct }) {
  ...
  return (<>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 12 }}><Sum label={t.s_income} value={money(income, loc)} color={C.ok} /><Sum label={t.s_expenses} value={money(expenses, loc)} color={C.bad} /><Sum label={t.s_net} value={money(income - expenses, loc)} color={income - expenses >= 0 ? C.ok : C.bad} /><Sum label={t.s_deductible} value={money(deductible, loc)} color={C.ok} hint /></div>
    ...
    {fseg === "accts" && <Card>{acctT.map(({ a, out, inc }) => (<div key={a.k} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 0", borderBottom: `1px solid ${C.borderSoft}` }}><div style={{ display: "flex", gap: 12, alignItems: "center" }}><span style={{ width: 34, height: 34, borderRadius: 9, background: C.surface2, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", color: C.gold }}><AcctI type={a.type} /></span><div><div style={{ fontSize: 15 }}>{a.label}</div><div style={{ fontSize: 11.5, color: C.mute, textTransform: "capitalize" }}>{a.type}</div></div></div><div style={{ textAlign: "right" }}>{inc > 0 && <div className="num" style={{ fontSize: 13.5, color: C.ok }}>+{money(inc, loc)}</div>}{out > 0 && <div className="num" style={{ fontSize: 13.5, color: C.dim }}>−{money(out, loc)}</div>}</div></div>))}</Card>}
...
function Places({ t, lang, places, onAdd, onEdit }) {
  const es = lang === "es";
  const [cat, setCat] = useState("all");
  const CATS3 = [["all", es ? "Todos" : "All"], ["restaurante", es ? "Restaurante" : "Restaurant"], ["cafe", "Café"], ["bar", "Bar"], ["coworking", "Coworking"], ["evento", es ? "Evento" : "Event"], ["otro", es ? "Otro" : "Other"]];
  const list = cat === "all" ? places : places.filter((p) => p.person === cat);
  const icon = (k) => ({ restaurante: "🍽️", cafe: "☕", bar: "🍸", coworking: "💼", evento: "🎪", otro: "📍" }[k] || "📍");
  return (<>
    <button onClick={onAdd} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, width: "100%", marginTop: 10, padding: "13px", borderRadius: 14, border: `1px dashed ${C.border}`, background: C.surface, color: C.gold, cursor: "pointer", fontFamily: SF, fontSize: 14.5, fontWeight: 600 }}>＋ {es ? "Agregar lugar" : "Add place"}</button>
    <div style={{ display: "flex", gap: 8, overflowX: "auto", marginTop: 12, paddingBottom: 2 }}>{CATS3.map(([k, lbl]) => <button key={k} onClick={() => setCat(k)} style={{ flexShrink: 0, padding: "7px 13px", borderRadius: 999, cursor: "pointer", fontFamily: SF, fontSize: 12.5, fontWeight: cat === k ? 600 : 400, background: cat === k ? C.surface2 : "transparent", color: cat === k ? C.text : C.mute, border: `1px solid ${cat === k ? C.border : "transparent"}`, whiteSpace: "nowrap" }}>{lbl}</button>)}</div>
    <div style={{ marginTop: 12 }}>
      {list.length === 0 ? <div style={{ padding: "24px 14px", textAlign: "center", color: C.mute, fontSize: 13.5 }}>{es ? "Sin lugares aún. Guarda tus favoritos de LA." : "No places yet. Save your LA favorites."}</div>
        : list.map((p) => (<div key={p.id} className="rise" onClick={() => onEdit(p)} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "13px 14px", background: C.surface, border: `1px solid ${C.borderSoft}`, borderRadius: 14, marginBottom: 9, cursor: "pointer" }}>
          <span style={{ fontSize: 22, flexShrink: 0, lineHeight: 1 }}>{icon(p.person)}</span>
          <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 14.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.title}</div>{p.detail ? <div style={{ fontSize: 12, color: C.mute, marginTop: 4, lineHeight: 1.45, whiteSpace: "pre-wrap", overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{p.detail}</div> : null}</div>
        </div>))}
    </div>
  </>);
}
  ...
  return (
    <div className="rise" onClick={onOpen} style={{ marginTop: 12, background: C.surface, border: `1px solid ${C.borderSoft}`, borderRadius: 18, padding: "16px 18px", fontFamily: SF, cursor: "pointer" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        ...
        <span style={{ fontSize: 11.5, color: entry ? C.ok : C.mute }}>{entry ? (es ? "✓ Escrito" : "✓ Done") : (es ? "30 seg" : "30 sec")}</span>
      </div>
  ...
  return (<>
    ...
    <div style={{ marginTop: 14 }}>
      {dates.length === 0 ? <div style={{ padding: "24px 14px", textAlign: "center", color: C.mute, fontSize: 13.5 }}>{es ? "Sin fechas aún. Agrega cumpleaños o aniversarios importantes." : "No dates yet. Add birthdays or anniversaries."}</div>
        : dates.map((d) => { const du = daysUntil(d.dateISO); const col = du == null ? C.mute : du <= 3 ? C.warn : du <= 14 ? C.dim : C.dim; return (<div key={d.id} className="rise" onClick={() => onEdit(d)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 14px", background: C.surface, border: `1px solid ${C.borderSoft}`, borderRadius: 14, marginBottom: 9, cursor: "pointer" }}>
  ...
  return (<>
    ...
    {upcoming.length > 0 && (<>
      ...
      <Card>{upcoming.map((r) => {
        ...
        const col = d < 0 ? C.bad : d <= 30 ? C.warn : C.dim;
...
function Capture({ t, lang, input, setInput, processCapture, processing, openVoice, openPhoto, recent }) {
  return (<div style={{ marginTop: 4 }}>
    ...
    {recent.length > 0 && <><SectionLabel>{t.cap_recent}</SectionLabel><Card>{recent.map((it) => (it.kind ? <div key={it.id} style={{ display: "flex", justifyContent: "space-between", padding: "11px 0", borderBottom: `1px solid ${C.borderSoft}` }}><div style={{ fontSize: 15 }}>{it.note}</div><div className="num" style={{ fontSize: 14, color: it.kind === "income" ? C.ok : C.text }}>{it.kind === "income" ? "+" : "−"}{money(it.amount, lang === "es" ? "es-MX" : "en-US")}</div></div> : <ItemRow key={it.id} it={it} lang={lang} sub={it.detail || it.dateLabel} />))}</Card></>}
  </div>);
}
