    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: SF, WebkitFontSmoothing: "antialiased" }}>
      ...
      <div style={{ maxWidth: contentMax, marginLeft: desktop ? `max(220px, calc(220px + (100% - ${contentMax + 220}px) / 2))` : "auto", marginRight: "auto", minHeight: "100vh", position: "relative", paddingBottom: desktop ? 40 : 86 }}>
        <div style={{ position: "sticky", top: 0, zIndex: 20, backdropFilter: "blur(14px)", background: "rgba(26,26,31,.72)", padding: "16px 20px 12px", borderBottom: `1px solid ${C.borderSoft}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>{desktop ? (<div style={{ fontSize: 20, fontWeight: 600, textTransform: "capitalize" }}>{t["nav_" + tab]}</div>) : (<><div style={{ display: "flex", alignItems: "center", gap: 8 }}><OcIcon size={22} ring="#8a9095" dot={C.gold} /><div style={{ fontSize: 22, fontWeight: 600, letterSpacing: "0.16em" }}>onucore<span style={{ color: C.gold, fontSize: 10, verticalAlign: "super", marginLeft: 3, fontWeight: 700 }}>AI</span></div></div><div style={{ fontSize: 9, letterSpacing: "0.26em", color: C.mute, marginTop: 2, textTransform: "uppercase" }}>{t["nav_" + tab]}</div></>)}</div>
...
function Money({ t, lang, loc, txns, obligations, subs, period, setPeriod, fseg, setFseg, recentId, onEditTxn, onEditItem, onAdd, onAddSub, onTogglePaid, onReport, setAsidePct }) {
  ...
  return (<>
    ...
    {fseg === "txns" && <Card>{view.length === 0 ? <Empty>{t.none_txns}</Empty> : view.map((x) => { const meta = x.kind === "income" ? incBy(x.cat) : catBy(x.cat); return (<div key={x.id} onClick={() => onEditTxn(x)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: `1px solid ${C.borderSoft}`, cursor: "pointer", background: x.id === recentId ? "rgba(229,72,77,.08)" : "transparent" }}><div style={{ display: "flex", gap: 11, alignItems: "center", minWidth: 0 }}><span style={{ width: 9, height: 9, borderRadius: 999, background: meta.color, flexShrink: 0 }} /><div style={{ minWidth: 0 }}><div style={{ fontSize: 15 }}>{x.note || meta[lang === "es" ? "es" : "en"]}</div><div style={{ fontSize: 11.5, color: C.mute, marginTop: 3 }}>{meta[lang === "es" ? "es" : "en"]} · {x.account === "personal" ? "Personal" : x.account === "business" ? "Oprinte" : acctBy(x.account).label} · {fmtDate(x.dateISO, lang)}{x.kind === "expense" && dedAmount(x) > 0 ? "  · ✓" : ""}</div></div></div><div className="num" style={{ fontSize: 15, color: x.kind === "income" ? C.green : C.text, flexShrink: 0 }}>{x.kind === "income" ? "+" : "−"}{money(x.amount, loc)}</div></div>); })}</Card>}
    ...
    {fseg === "subs" && (() => { const norm = (s) => { const a = Number(s.amount) || 0; return s.dateLabel === "yearly" ? a / 12 : s.dateLabel === "weekly" ? (a * 52) / 12 : a; }; const moTotal = subs.reduce((sum, s) => sum + norm(s), 0); const cyl = (s) => (s.dateLabel === "yearly" ? t.sub_yearly : s.dateLabel === "weekly" ? t.sub_weekly : t.sub_monthly); return (<><div style={{ display: "flex", gap: 8, marginBottom: 12 }}><PlaidConnect accountLabel="personal" label="🏦 Personal" onDone={() => { if (typeof window !== "undefined") window.location.reload(); }} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, flex: 1, padding: "12px", borderRadius: 12, border: "none", background: C.gold, color: "#ffffff", cursor: "pointer", fontFamily: SF, fontSize: 13.5, fontWeight: 600 }} /><PlaidConnect accountLabel="business" label="🏦 Oprinte" onDone={() => { if (typeof window !== "undefined") window.location.reload(); }} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, flex: 1, padding: "12px", borderRadius: 12, border: `1px solid ${C.goldSoft}`, background: "transparent", color: C.gold, cursor: "pointer", fontFamily: SF, fontSize: 13.5, fontWeight: 600 }} /></div><PlaidAccounts lang={lang} C={C} SF={SF} /><button onClick={async () => { const r = await fetch("/api/plaid/sync", { method: "POST" }); if (typeof window !== "undefined" && r.ok) window.location.reload(); }} style={{ display: "block", width: "100%", background: "transparent", border: "none", color: C.mute, fontSize: 12, cursor: "pointer", fontFamily: SF, marginTop: -8, marginBottom: 12 }}>↻ {lang === "es" ? "Sincronizar (si ya conectaste el banco)" : "Sync (if bank already connected)"}</button><button onClick={async () => { if (typeof window !== "undefined" && !window.confirm(lang === "es" ? "¿Desconectar el banco y borrar los datos importados?" : "Disconnect bank and delete imported data?")) return; await fetch("/api/plaid/disconnect", { method: "POST" }); if (typeof window !== "undefined") window.location.reload(); }} style={{ display: "block", width: "100%", background: "transparent", border: "none", color: C.mute, fontSize: 11.5, cursor: "pointer", fontFamily: SF, marginTop: -6, marginBottom: 12 }}>{lang === "es" ? "Desconectar / limpiar banco" : "Disconnect / clear bank"}</button><button onClick={async () => { if (typeof window !== "undefined" && !window.confirm(lang === "es" ? "¿Intercambiar Personal ↔ Oprinte?" : "Swap Personal ↔ Oprinte?")) return; await fetch("/api/plaid/swap", { method: "POST" }); if (typeof window !== "undefined") window.location.reload(); }} style={{ display: "block", width: "100%", background: "transparent", border: "none", color: C.mute, fontSize: 11.5, cursor: "pointer", fontFamily: SF, marginTop: -6, marginBottom: 12 }}>{lang === "es" ? "Intercambiar Personal ↔ Oprinte" : "Swap Personal ↔ Oprinte"}</button><div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", padding: "2px 2px 12px" }}><span style={{ fontSize: 12.5, color: C.dim }}>{subs.length} {t.seg_subs.toLowerCase()}</span><span className="num" style={{ fontSize: 22, fontWeight: 600 }}>{money(moTotal, loc)} <span style={{ fontSize: 12, color: C.mute, fontWeight: 400 }}>{t.sub_amonth}</span></span></div><Card>{subs.length === 0 ? <Empty>{t.none_subs}</Empty> : subs.map((s) => (<div key={s.id} onClick={() => onEditItem(s)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: `1px solid ${C.borderSoft}`, cursor: "pointer", background: s.id === recentId ? "rgba(229,72,77,.08)" : "transparent" }}><div style={{ minWidth: 0 }}><div style={{ fontSize: 15 }}>{s.title}</div><div style={{ fontSize: 11.5, color: C.mute, marginTop: 3 }}>{cyl(s)}{s.dateISO ? ` · ${t.sub_renews} ${fmtDate(s.dateISO, lang)}` : ""}</div></div><div className="num" style={{ fontSize: 15, flexShrink: 0 }}>{money(Number(s.amount) || 0, loc)}</div></div>))}</Card></>); })()}
  </>);
}
