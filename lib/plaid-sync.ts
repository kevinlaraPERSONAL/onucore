...
// Shared Plaid sync engine. Callable with the user's session client (manual /
// app-open sync) or the service-role client (Plaid webhook), always scoped to
// one user. Pulls accounts, the full fiscal year of transactions (paginated),
// and recurring charges → subscriptions. De-dupes and re-categorizes.

// Any Supabase client (session or service-role); queries always filter by user_id.
/* eslint-disable @typescript-eslint/no-explicit-any */
type DB = { from: (table: string) => any };

export const PLAID_WEBHOOK_URL = process.env.PLAID_WEBHOOK_URL || "https://onucore.app/api/plaid/webhook";
...
export async function runPlaidSync(db: DB, userId: string) {
  const { data: items } = await db.from("plaid_items").select("*").eq("user_id", userId);
  if (!items || !items.length) return { error: "no_bank", subsAdded: 0, txnsAdded: 0, errors: [] as string[] };

  const client = plaidClient();

  // Per-account tags (personal | business). A single login can hold both.
  const { data: accRows } = await db.from("plaid_accounts").select("account_id, label").eq("user_id", userId);
  const accMap = new Map<string, string>(
    (accRows || []).map((a: { account_id: string; label: string }) => [a.account_id, a.label]),
  );

  const { data: existSubs } = await db
    .from("items")
    .select("title")
    .eq("user_id", userId)
  ...
  for (const it of items) {
    // Make sure Plaid pings our webhook when this bank has new transactions.
    try {
      await client.itemWebhookUpdate({ access_token: it.access_token, webhook: PLAID_WEBHOOK_URL });
    } catch {
      /* best effort */
    }
    ...
    try {
      ...
      if (rows.length) {
        ...
        if (fresh.length) {
          const ins = await db.from("txns").insert(fresh);
  ...
  return { subsAdded, txnsAdded, errors };
}
